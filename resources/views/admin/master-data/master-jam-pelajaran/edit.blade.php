    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header border-bottom">
                <h3 class="h4 mb-0">Edit Jam Pelajaran</h3>
            </div>
            <div class="card-body">
                <form action="{{ route('m_jamupdate', $jam_pel_edit->id) }}" method="POST" class="form-horizontal">
                    @csrf
                    @method('put')
                        ...
                        <div class="col-sm-9">
                            <select class=" form-select @error('paket') is-invalid @enderror" name="paket">
                                ...
                                @foreach ($data_paket as $d_paket)
                                    <option value="{{ $d_paket->id }}"
                                        {{ isset($jam_pel_edit) ? ($d_paket->id == $jam_pel_edit->id_paket ? 'selected' : '') : '' }}>
                                        {{ $d_paket->jenis_paket }}</option>
                                @endforeach
                            </select>
                            @error('paket')
                        ...
                        <div class="col-sm-9">
                            <select class="js-example-responsive form-select @error('hari') is-invalid @enderror"
                                ...
                                @foreach ($data_hari as $d_hari)
                                    <option value="{{ $d_hari->id }}"
                                        {{ isset($jam_pel_edit) ? (in_array($d_hari->id, $decode_hari) ? 'selected' : '') : '' }}>
                                        {{ $d_hari->nama_hari }}</option>
                                @endforeach
                            </select>
                            @error('hari')
                                <small class="form-text text-danger">{{ $message }}</small>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <input class="form-control @error('jam_mulai') is-invalid @enderror" name="jam_mulai"
                                ...
                                value="{{ old('jam_mulai', $jam_pel_edit->jam_mulai) }}">
                            @error('jam_mulai')
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <input class="form-control @error('jam_selesai') is-invalid @enderror" name="jam_selesai"
                                ...
                                value="{{ old('jam_selesai', $jam_pel_edit->jam_selesai) }}">
                            @error('jam_selesai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
